...
image1 = pygame.image.load('SEASONLOGO').convert()
...
while repeat:
        ...
        if event.type == pygame.KEYDOWN:
            if event.key == pygame.K_q:
                ...
                if (curView == 1):
                    ...
                    player1 = OMXPlayer(stream2, args=['--live'])
                player1.set_video_crop(crop1x1, crop1y1, crop1x2, crop1y2)
                player1.set_video_pos(win1x1,win1y1,win1x2,win1y2)
                player1.play()
                # player2 = OMXPlayer(stream2, dbus_name='org.mpris.MediaPlayer2.omxplayer1', args=['--live'])
    ...
    if (newView != curView):
        if (newView == 1):
            ...
            serverSock.sendto(b'1', ('192.168.123.32',UDP_PORT_NO))
            serverSock.sendto(b'1', ('192.168.123.33',UDP_PORT_NO))
            serverSock.sendto(b'1', ('192.168.123.37',UDP_PORT_NO))
            serverSock.sendto(b'1', ('192.168.123.38',UDP_PORT_NO))
        elif (newView == 2):
            serverSock.sendto(b'2', ('192.168.123.29',UDP_PORT_NO))
            serverSock.sendto(b'2', ('192.168.123.32',UDP_PORT_NO))
            serverSock.sendto(b'2', ('192.168.123.33',UDP_PORT_NO))
            serverSock.sendto(b'2', ('192.168.123.37',UDP_PORT_NO))
            serverSock.sendto(b'2', ('192.168.123.38',UDP_PORT_NO))

        player1.quit()
        time.sleep(1.5)
        curView = newView
        if (newView == 1):
            # UDP is unreliable, so send it again just in case
            serverSock.sendto(b'1', ('192.168.123.29',UDP_PORT_NO))
            serverSock.sendto(b'1', ('192.168.123.32',UDP_PORT_NO))
            serverSock.sendto(b'1', ('192.168.123.33',UDP_PORT_NO))
            serverSock.sendto(b'1', ('192.168.123.37',UDP_PORT_NO))
